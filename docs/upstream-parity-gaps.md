...
Status: **LOCAL GREEN / runtime candidate pending hosted CI**. The range changes **9** `packages/ai` paths (`+128/-23`): package/changelog metadata, generator metadata, OpenAI Responses cache serialization, image models, type comments, and 3 modified tests (`cache-retention`, `max-thinking`, `supports-xhigh`). Whole-corpus filename evidence is in `docs/v0851-142-test-crosswalk.md`: **142/142** upstream `packages/ai/test/*.test.ts` files accounted, with executable validator `scripts/validate_v0851_manifests.py` and unchanged corpus hash `56f8742065a4ad01d73e5aee53035324f2e7333a735222ab15db870819e29065`.
