...
mod tests {
    ...
    fn gets_all_available_bedrock_models() {
        assert!(!list_models(Some("amazon-bedrock")).is_empty(), "the Bedrock catalog must be non-empty");
    }
    ...
    fn builtin_image_models_register_the_openrouter_provider_with_its_catalog() {
        ...
        assert!(models.iter().all(|m| m.api == "openrouter-images"), "all image models use the openrouter-images api");
        assert!(models.iter().all(|m| m.provider == "openrouter"), "the only built-in image provider is openrouter");
    }
}
