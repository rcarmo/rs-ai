#!/usr/bin/env python3
"""Validate v0.84.4 release audit manifests."""
...
CROSSWALK = ROOT / "docs/v0844-137-test-crosswalk.md"
EXPECTED_CHANGED_PATHS = ['packages/ai/CHANGELOG.md', 'packages/ai/package.json', 'packages/ai/scripts/generate-models.ts', 'packages/ai/scripts/openrouter-reasoning-options.ts', 'packages/ai/src/api/mistral-conversations.ts', 'packages/ai/src/api/openai-completions.ts', 'packages/ai/src/image-models.generated.ts', 'packages/ai/src/providers/cloudflare-ai-gateway.ts', 'packages/ai/src/types.ts', 'packages/ai/test/fireworks-models.test.ts', 'packages/ai/test/mistral-http-transport.test.ts', 'packages/ai/test/openai-completions-reasoning-details.test.ts', 'packages/ai/test/openai-completions-tool-choice.test.ts', 'packages/ai/test/openrouter-reasoning-options.test.ts', 'packages/ai/test/zai-coding-plan-models.test.ts']
...
def crosswalk_rows() -> dict[str, str]:
    ...
    for line in CROSSWALK.read_text().splitlines():
        if line.startswith("## Per-file 137-test disposition matrix"):
            in_matrix = True
            continue
...
def main() -> int:
    ...
    if missing or extra:
        failures.append(f"v0.84.4 changed-path matrix mismatch\nmissing={missing}\nextra={extra}")
    rows = crosswalk_rows()
    ...
    if len(rows) != 137 or missing_tests or extra_tests:
        failures.append(f"v0.84.4 137-test crosswalk mismatch\nrows={len(rows)} missing={missing_tests} extra={extra_tests}")
    ...
    if failures:
        ...
        return 1
    print("v0.84.4 manifests verified: changedPaths=15 testRows=137")
    return 0
