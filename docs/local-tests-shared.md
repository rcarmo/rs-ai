...
Status: **ACCEPTED / completed for bounded deterministic v0.85.1 release parity**. The v0.85.1 audit spans 9 changed `packages/ai` paths and **3 changed test paths total**: modified `cache-retention.test.ts`, `max-thinking.test.ts`, and `supports-xhigh.test.ts`. The final upstream test corpus is unchanged at **142 files**, recorded in `docs/v0851-142-test-crosswalk.md` and mechanically checked by `scripts/validate_v0851_manifests.py` against changed-path hash `ee26f669d92dc77b265731165a2ff69ccb67defba92517cbbd5f97a186e187d2` and corpus hash `56f8742065a4ad01d73e5aee53035324f2e7333a735222ab15db870819e29065`.

Named tests in `src/tests/providers/openai/openai_responses_prompt_cache_test.rs` cover v0.85.1 OpenAI Responses cache serialization through the production payload builder: explicit prompt-cache mode for `cacheRetention: none`, `ttl: "30m"` for supported long retention, legacy `prompt_cache_retention: "24h"` for older models, and omission cases. `src/tests/release/v0851_release_test.rs`, `src/tests/catalogs/max_thinking_test.rs`, and `src/tests/catalogs/supports_xhigh_test.rs` cover GPT-6 Astra OpenAI/Codex/Azure/OpenCode/GitHub Copilot/OpenRouter/Vercel catalog aliases, cost/tier/window/input metadata, tool-search/additional-tool flags, explicit prompt cache compat, and xhigh/max thinking behavior. Metadata verifier targets pinned tarball SHA `af7d11986179445ce6fe88b37d57de22f823c0ffd3a65cae31c555b7f5e99253` and reports text `1354`, providers `39`, APIs `9`, batch aliases `68`, image `52`; manifest and baseline-delta validators include hash/fault and `+20/-2/18` text plus `+2/-0/0` image delta gates. `scripts/verify_generated_reproducibility.py` proves two independent catalog generations are byte-for-byte identical using the release manifest `generatedAt` header. Final local gates pass: all-feature tests x3 plus summary (`993 passed`, `0 ignored`), no-default (`853 passed`, doctest `1 passed`), no-default+Bedrock (`993 passed`, doctest `1 passed`), strict fmt/build/clippy, release metadata (`17 passed`), provider/id comparator (`1354/1354`, `52/52`), manifest/metadata/baseline fault gates, and SBOM/license/RustSec (`278` components, local SBOM SHA-256 `79a83032828328745a28fd37eb71c39aeeccf99dc728a715c301367f1a45ad46`) after updating `Cargo.lock` from `rustls 0.23.43` to `rustls 0.23.45` for `RUSTSEC-2026-0285`. Hosted CI run `35156144749` passed for runtime candidate `e74bfc2c3dc1e12c8b4cedf8e2f8e26c13f299de`, with SBOM artifact `10471770993`, inner SBOM SHA-256 `87cf86e01efc33eb9242da6d54eec4f37354e26ac5bcfa07d398b0f65d41082a`, embedded revision `e74bfc2c3dc1e12c8b4cedf8e2f8e26c13f299de`, and `278` components. Superseded candidate `1b624cfce3dc2c3b9ed534b8c73305a3589896af` failed hosted RustSec on `RUSTSEC-2026-0285` and was replaced by the `rustls 0.23.45` lockfile candidate. README/catalog prose is updated in a post-acceptance docs-only commit; durable `upstream-v0.85.1` release assets are handled by the guarded publisher workflow.


## v0.85.0 bounded release evidence (historical)

Source: upstream tag `107d79f11072bbc8a3a757ed7fd69596bee7d68c` (`v0.85.0`), exact release-only delta `b79e4cc834970cca69daebffab7df1da7d1e52c4..107d79f11072bbc8a3a757ed7fd69596bee7d68c`.

Status: **ACCEPTED / completed for bounded deterministic v0.85.0 release parity**. The v0.85.0 audit spans 51 changed `packages/ai` paths and **29 changed test paths total**: 22 existing tests modified, 6 new tests, and 1 deleted Cloudflare gateway binding test. The final upstream test corpus is **142 files**, recorded in `docs/v0850-142-test-crosswalk.md` and mechanically checked by `scripts/validate_v0850_manifests.py`.

Named tests in `src/tests/release/v0850_release_test.rs` cover v0.85.0 catalog counts, `providerThinkingLevel` serialization, OpenAI-compatible vLLM priority, Responses `supportsMaxOutputTokens`, explicit UUIDv7 timestamps, and NO_PROXY suffix/port behavior. `src/tests/providers/anthropic/anthropic_mid_conversation_effort_test.rs` covers managed Anthropic effort metadata/betas/block binding, `src/tests/core/pi_messages_test.rs` covers pi-messages `providerThinkingLevel`, `src/tests/core/assistant_message_frame_test.rs` covers the new public frame encoder/reducer plus exact camelCase frame JSON, assistant-only required start partials without `isError`, strict start decode, presence-aware redaction, no-null optionals, legacy decode aliases, empty vs absent optional metadata, and decode-time malformed nested shapes. `src/tests/providers/openai/openai_responses_terminal_event_test.rs` covers Responses stale-error terminal cleanup plus serialized `errorMessage` omission/retention, `src/tests/providers/other/pre_generation_error_test.rs` covers explicit production pre-generation auth errors, and `src/tests/providers/codex/openai_codex_stream_test.rs` covers terminal SSE without a trailing blank line. Metadata verifier targets pinned tarball SHA `46188bdacb555a07466a0111f3963f20932a16199e4d6cfb8d44a7fe5fc6e342` and reports text `1336`, providers `39`, APIs `9`, batch aliases `66`, image `50`; manifest and baseline-delta validators include hash/fault and `+72/-26/79` text-delta gates. Strict frame-decode runtime SHA `0eb50d428d75a0281231fcd294d768c3db9cd17c` passed hosted CI run `33896845145` with SBOM artifact `rs-ai-sbom-0eb50d428d75a0281231fcd294d768c3db9cd17c` (`9946095734`) and inner CI SBOM SHA-256 `24256dacc79097c90f399632852967c868371d8f64c85a5c7dea91ca3bd17284`.
