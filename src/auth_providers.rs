...
impl OAuthAuth for CodexOAuth {
    async fn refresh(&self, credential: &OAuthCredential) -> Result<OAuthCredential, ModelsError> {
        let refresh = credential.refresh.as_deref()
...
mod tests {
    use super::*;
    use crate::auth::{
        resolve_provider_auth, ApiKeyCredential, Credential, EnvAuthContext,
    ...
    fn model(provider: &str) -> Model {
        Model {
            id: "m".into(), name: "M".into(), api: "openai-codex-responses".into(),
    ...
    async fn codex_oauth_refreshes_expired_credential_through_resolver() {
        ...
        let payload = serde_json::json!({ "https://api.openai.com/auth": {"chatgpt_account_id": "acc_123"} });
        ...
        store.modify::<_, _, std::convert::Infallible>("openai-codex", |_| async {
            Ok(Some(Credential::OAuth(OAuthCredential {
                access: "stale".into(), refresh: Some("old".into()), expires: now_millis() - 1, account_id: None,
            })))
        }).await.unwrap();

        let provider = ProviderAuth {
            api_key: None,
            oauth: Some(Box::new(CodexOAuth { token_url: Some(format!("{}/oauth/token", server.uri())) })),
        };
        let ctx = EnvAuthContext::new();
        let result = resolve_provider_auth("openai-codex", &provider, &model("openai-codex"), &store, &ctx, None)
            .await.unwrap().unwrap();
    ...
    async fn codex_oauth_missing_refresh_token_errors() {
        ...
        let err = oauth.refresh(&OAuthCredential {
            access: "a".into(), refresh: None, expires: 0, account_id: None,
        }).await.unwrap_err();
    ...
    async fn stored_api_key_credential_still_resolves_when_provider_has_oauth_only_for_other_path() {
        ...
        store.modify::<_, _, std::convert::Infallible>("openai", |_| async {
            Ok(Some(Credential::ApiKey(ApiKeyCredential { key: Some("sk".into()), env: None })))
        }).await.unwrap();
        struct PassThrough;
        #[async_trait::async_trait]
        impl crate::auth::ApiKeyAuth for PassThrough {
            async fn resolve(&self, _m: &Model, _c: &dyn crate::auth::AuthContext, cred: Option<&ApiKeyCredential>)
                -> Result<Option<crate::auth::AuthResult>, ModelsError> {
                Ok(cred.and_then(|c| c.key.clone()).map(|k| crate::auth::AuthResult {
                    auth: ModelAuth { api_key: Some(k), ..Default::default() }, env: None, source: None,
                }))
            }
        }
        let provider = ProviderAuth { api_key: Some(Box::new(PassThrough)), oauth: None };
        let ctx = EnvAuthContext::new();
