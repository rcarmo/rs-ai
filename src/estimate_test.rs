...
mod tests {
    ...
    fn usage(input: u32, output: u32, total: u32) -> Usage {
        Usage { input, output, total_tokens: total, ..Default::default() }
    }

    fn msg(role: Role, content: Vec<ContentBlock>, u: Option<Usage>, stop: Option<StopReason>) -> Message {
        Message {
            role, content, timestamp: 0, api: None, provider: None, model: None,
            response_id: None, response_model: None, diagnostics: Vec::new(), usage: u,
            stop_reason: stop, error_message: None, tool_call_id: None, tool_name: None,
            is_error: false, details: None,
        }
    }

    fn text(s: &str) -> ContentBlock { ContentBlock::Text { text: s.into(), text_signature: None } }
    ...
    fn content_tokens_weight_images_at_4800() {
        ...
        let content = vec![text("abcd"), ContentBlock::Image { data: "x".into(), mime_type: "image/png".into() }];
    ...
    fn assistant_message_counts_text_thinking_and_toolcall_json() {
        ...
        let m = msg(Role::Assistant, vec![
            text("hello"), // 5
            ContentBlock::Thinking { thinking: "think".into(), thinking_signature: None, redacted: false }, // 5
            ContentBlock::ToolCall { id: "1".into(), name: "fn".into(), arguments: args, thought_signature: None }, // 2 + len({"a":1})=7
        ], None, None);
    ...
    fn context_anchors_on_last_assistant_usage_and_adds_trailing() {
        let messages = vec![
            msg(Role::User, vec![text("ignored because anchored")], None, None),
            msg(Role::Assistant, vec![text("done")], Some(usage(0, 0, 100)), Some(StopReason::Stop)),
            msg(Role::User, vec![text("abcd")], None, None), // trailing: ceil(4/4)=1
        ];
        let ctx = Context { system_prompt: Some("sys".into()), tools: Vec::new(), messages };
    ...
    fn context_without_usage_adds_system_prefix() {
        ...
        let messages = vec![
            msg(Role::Assistant, vec![text("x")], Some(usage(0, 0, 500)), Some(StopReason::Aborted)),
            msg(Role::User, vec![text("abcd")], None, None), // 1 token
        ];
        let ctx = Context { system_prompt: Some("12345678".into()), tools: Vec::new(), messages }; // sys = 2 tokens
        let est = estimate_context_tokens(&ctx);
