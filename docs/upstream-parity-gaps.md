...
Status: **ACCEPTED / completed for bounded deterministic v0.85.1 release parity**. The range changes **9** `packages/ai` paths (`+128/-23`): package/changelog metadata, generator metadata, OpenAI Responses cache serialization, image models, type comments, and 3 modified tests (`cache-retention`, `max-thinking`, `supports-xhigh`). Whole-corpus filename evidence is in `docs/v0851-142-test-crosswalk.md`: **142/142** upstream `packages/ai/test/*.test.ts` files accounted, with executable validator `scripts/validate_v0851_manifests.py` and unchanged corpus hash `56f8742065a4ad01d73e5aee53035324f2e7333a735222ab15db870819e29065`.

Key v0.85.1 dispositions: generated text catalog is regenerated to **1354** provider/id pairs across **39** providers and **9** APIs, batch aliases are **68**, and image catalog is **52**. Runtime adaptations cover OpenAI Responses `prompt_cache_options` serialization for explicit prompt-cache models (`mode: "explicit"` for `none`, `ttl: "30m"` for supported long retention, no concurrent legacy retention), while older models keep `prompt_cache_retention: "24h"`. GPT-6 Astra is represented across OpenAI Responses, Azure OpenAI Responses, OpenAI Codex, OpenCode, GitHub Copilot, OpenRouter, and Vercel AI Gateway generated aliases with v0.85.1 cost/tier/window/input/tool-search/additional-tool/thinking metadata. Catalog headers now derive from release manifest `generatedAt` and are covered by a generate-twice byte-for-byte reproducibility gate.

Catalog evidence: npm tarball SHA-256 `af7d11986179445ce6fe88b37d57de22f823c0ffd3a65cae31c555b7f5e99253`; provider manifest structure hash `ff87cfcb3c1decb7ceeb4a5d71282696e108d093b7098f372d6f8a442dfed40d`; provider manifest SHA-256 `30e7f58cc33d5901dbcb64f0e00d0133620005737ffb73553b358ca23572bac8`; full metadata verifier `metadata verified: text=1354 providers=39 apis=9 batchAliases=68 image=52`; exact manifest hashes `ee26f669d92dc77b265731165a2ff69ccb67defba92517cbbd5f97a186e187d2` and `56f8742065a4ad01d73e5aee53035324f2e7333a735222ab15db870819e29065`; baseline delta verifier `text=+20/-2/18 changed image=+2/-0/0 changed`. Local gates pass: focused OpenAI Responses cache (`7 passed`), GPT-6 Astra release coverage (`5 passed`), max-thinking (`4 passed`), supports-xhigh (`17 passed`), simple-options (`18 passed`), release metadata (`17 passed`), full all-feature tests x3 plus summary (`993 passed`, `0 ignored`), no-default (`853 passed`, doctest `1 passed`), no-default+Bedrock (`993 passed`, doctest `1 passed`), strict Clippy/build/fmt, provider/id comparator (`1354/1354`, `52/52`), manifest/metadata/baseline fault gates, generated reproducibility, and SBOM/license/RustSec (`278` components, local SBOM SHA-256 `79a83032828328745a28fd37eb71c39aeeccf99dc728a715c301367f1a45ad46`) after updating `Cargo.lock` from `rustls 0.23.43` to `rustls 0.23.45` for `RUSTSEC-2026-0285`. Hosted CI run `35156144749` passed for runtime candidate `e74bfc2c3dc1e12c8b4cedf8e2f8e26c13f299de`, with SBOM artifact `10471770993`, inner SBOM SHA-256 `87cf86e01efc33eb9242da6d54eec4f37354e26ac5bcfa07d398b0f65d41082a`, embedded revision `e74bfc2c3dc1e12c8b4cedf8e2f8e26c13f299de`, and `278` components. Superseded candidate `1b624cfce3dc2c3b9ed534b8c73305a3589896af` failed hosted RustSec on `RUSTSEC-2026-0285` and was replaced by the `rustls 0.23.45` lockfile candidate.


## Historical parity audit: `@earendil-works/pi-ai` v0.85.0
