...
mod tests {
    ...
    fn model(context_window: u32) -> Model {
        Model {
            id: "m".into(), name: "m".into(), api: "openai-completions".into(), provider: "p".into(),
            base_url: "https://x".into(), reasoning: false, thinking_level_map: None, input: vec!["text".into()],
            cost: ModelCost::default(), context_window, max_tokens: 4096, headers: None, api_key: None, compat: Default::default(),
        }
    }
    ...
    fn error_message(err: &str) -> Message {
        Message {
            ...
            api: Some("openai-completions".into()), provider: Some("ollama".into()), model: Some("qwen3.5:35b".into()),
            response_id: None, response_model: None, diagnostics: Vec::new(),
            usage: Some(Usage::default()),
            stop_reason: Some(StopReason::Error), error_message: Some(err.into()),
    ...
    fn detects_explicit_ollama_prompt_too_long() {
        assert!(overflow(&error_message("400 `prompt too long; exceeded max context length by 100918 tokens`"), 32768));
    }

    #[test]
    fn detects_together_ai_context_length_errors() {
        assert!(overflow(&error_message("400 The input (516368 tokens) is longer than the model's context length (262144 tokens)."), 262144));
    }

    #[test]
    fn detects_litellm_wrapped_openai_maximum_context_length() {
        assert!(overflow(&error_message("Error: 503 litellm.ServiceUnavailableError: litellm.MidStreamFallbackError: litellm.APIConnectionError: APIConnectionError: OpenAIException - Requested token count exceeds the model's maximum context length of 131072 tokens."), 131072));
    }

    #[test]
    fn detects_openai_compatible_parenthesized_maximum_context_length() {
        assert!(overflow(&error_message("Error: 400 Input length (265330) exceeds model's maximum context length (262144)."), 262144));
    }

    #[test]
    fn detects_openrouter_poolside_maximum_allowed_input_length() {
        assert!(overflow(&error_message("Provider returned error: Input length 131393 exceeds the maximum allowed input length of 131040 tokens."), 131072));
    }

    #[test]
    fn ignores_generic_non_overflow_ollama_errors() {
        assert!(!overflow(&error_message("500 `model runner crashed unexpectedly`"), 32768));
    }

    #[test]
    fn ignores_bedrock_throttling_too_many_tokens() {
        assert!(!overflow(&error_message("Throttling error: Too many tokens, please wait before trying again."), 200000));
    }

    #[test]
    fn ignores_bedrock_service_unavailable() {
        assert!(!overflow(&error_message("Service unavailable: The service is temporarily unavailable."), 200000));
    }

    #[test]
    fn ignores_generic_rate_limit_errors() {
        assert!(!overflow(&error_message("Rate limit exceeded, please retry after 30 seconds."), 200000));
    }

    #[test]
    fn ignores_http_429_style_errors() {
        assert!(!overflow(&error_message("Too many requests. Please slow down."), 200000));
    }
