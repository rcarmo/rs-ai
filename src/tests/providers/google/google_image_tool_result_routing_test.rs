...
mod tests {
    ...
    fn contents(m: &Model) -> Vec<Value> {
        build_google_payload_public(m, &ctx(m), &StreamOptions::default())["contents"]
            .as_array()
            .unwrap()
            .clone()
    }
    ...
    fn keeps_separate_synthetic_image_turn_for_gemini_2x_google_api_models() {
        ...
        assert_eq!(
            c[3]["parts"][0]["text"],
            serde_json::json!("Tool result image:")
        );
    ...
    fn nests_image_tool_results_for_gemini_3_google_api_models() {
        ...
        let parts = image_response["parts"]
            .as_array()
            .expect("nested functionResponse.parts");
