...
mod tests {
    ...
    fn xiaomi_omits_mimo_v2_flash_from_token_plan_providers() {
        for provider in [
            ...
            let ids: Vec<String> = list_models(Some(provider))
                .iter()
                .map(|m| m.id.clone())
                .collect();
            assert!(
                !ids.iter().any(|id| id == "mimo-v2-flash"),
                "{provider} must omit mimo-v2-flash"
            );
            assert!(
                !ids.iter().any(|id| id == "mimo-v2-omni"),
                "{provider} must omit mimo-v2-omni"
            );
        }
    }
}
