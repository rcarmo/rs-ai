...
mod tests {
    use crate::auth::{
        ApiKeyAuth, ApiKeyCredential, AuthContext, AuthResult, Credential, EnvAuthContext,
        InMemoryCredentialStore, ModelsError, ModelsErrorCode, OAuthAuth, OAuthCredential,
    ...
    async fn stored_credential_without_matching_handler_blocks_ambient() {
        ...
        let provider = ProviderAuth {
            api_key: Some(Box::new(EnvKeyAuth)),
            oauth: None,
        };
        let ctx = EnvAuthContext::new();
        let r = resolve_provider_auth("p1", &provider, &model(), &store, &ctx, None)
            .await
            .unwrap();
        assert!(
            r.is_none(),
            "an unhandled stored credential must block ambient fallback"
        );
    }
    ...
    async fn serializes_concurrent_oauth_refreshes_no_double_refresh() {
        ...
        assert_eq!(
            ...
            1,
            "the per-provider lock prevents a double refresh"
        );
    ...
    async fn wraps_api_key_auth_failures_in_models_error_auth() {
        let store = InMemoryCredentialStore::new();
        let provider = ProviderAuth {
            api_key: Some(Box::new(FailingApiKey)),
            oauth: None,
        };
        let ctx = EnvAuthContext::new();
        let err = resolve_provider_auth("p1", &provider, &model(), &store, &ctx, None)
    ...
    fn merges_resolved_auth_into_options_explicit_wins_per_field() {
        ...
        let (m2, o2) =
            merge_auth_into_request(&auth, model(), crate::types::StreamOptions::default());
