...
Status: **ADAPTED / completed for bounded deterministic v0.85.0 release parity; hosted CI passed for runtime candidate; auditor acceptance pending**. The range changes **51** `packages/ai` paths: 19 source/scripts paths, 29 tests, and 3 package/docs paths. Whole-corpus filename evidence is in `docs/v0850-142-test-crosswalk.md`: **142/142** upstream `packages/ai/test/*.test.ts` files accounted, with executable validator `scripts/validate_v0850_manifests.py`.

Key v0.85.0 dispositions: generated text catalog is regenerated to **1336** provider/id pairs across **39** providers and **9** APIs, batch aliases are **66**, and image catalog remains **50**. Runtime adaptations cover `providerThinkingLevel`, Anthropic managed mid-conversation effort/binding, OpenAI-compatible vLLM priority, Responses `supportsMaxOutputTokens`, UUID explicit timestamps, pi-messages metadata, Codex terminal SSE without trailing blank line, and NO_PROXY suffix/port behavior. Cloudflare Workers binding replacement is documented as N/A for Rust Workers binding-object semantics, with HTTP/gateway semantics covered elsewhere.

Catalog evidence: npm tarball SHA-256 `46188bdacb555a07466a0111f3963f20932a16199e4d6cfb8d44a7fe5fc6e342`; provider manifest structure hash `a71a055905e4b12c9bb41fa6c2bf90fb2944ca76843bdfd3831377caba1be189`; full metadata verifier `metadata verified: text=1336 providers=39 apis=9 batchAliases=66 image=50`. Final local gates: fmt/build, strict all-feature clippy, three post-lock all-target/all-feature test passes (`947 passed`, `0 ignored`), metadata fault gates, provider/id comparator, v0.85.0 and v0.84.4 manifest validators, no-default/bedrock feature checks, SBOM validation, SPDX review, and RustSec wrapper passed. Runtime candidate SHA `ae87c229cd0a76a3b1611279ebcb3ef4ddd4d0fa` passed hosted GitHub Actions run `33888972744`, job `build-test-lint` (`101075680548`), with SBOM artifact `rs-ai-sbom-ae87c229cd0a76a3b1611279ebcb3ef4ddd4d0fa` (`9943241339`) retained until `2026-10-04T15:27:11Z`; final CI-evidence docs SHA is recorded separately.
