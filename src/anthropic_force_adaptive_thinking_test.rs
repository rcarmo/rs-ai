...
mod tests {
    ...
    use crate::types::{Context, ContentBlock, Message, Model, ModelCost, ModelCompat, Role, StreamOptions, ThinkingLevel};
    ...
    fn custom_model(compat: ModelCompat) -> Model {
        Model {
            id: "vendor--claude-opus-latest".into(), name: "Vendor Proxy Opus Latest".into(),
            api: "anthropic-messages".into(), provider: "vendor-proxy".into(), base_url: "http://127.0.0.1:9".into(),
            reasoning: true, thinking_level_map: None, input: vec!["text".into()], cost: ModelCost::default(),
            context_window: 200000, max_tokens: 32000, headers: None, api_key: None, compat,
        }
    }
    ...
    fn sends_legacy_thinking_payload_for_custom_model_ids_by_default() {
        let p = payload(&custom_model(ModelCompat::default()), Some(ThinkingLevel::Medium));
    ...
    fn sends_adaptive_thinking_payload_when_force_adaptive_thinking_true() {
        let compat = ModelCompat { force_adaptive_thinking: Some(true), ..Default::default() };
    ...
    fn allows_builtin_adaptive_models_to_opt_out_with_force_adaptive_thinking_false() {
        ...
        model.compat = ModelCompat { force_adaptive_thinking: Some(false), ..Default::default() };
