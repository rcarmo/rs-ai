...
mod tests {
    ...
    fn model() -> Model {
        Model {
            id: "us.anthropic.claude-sonnet-4-5-20250929-v1:0".into(), name: "Sonnet 4.5".into(),
            api: "bedrock-converse-stream".into(), provider: "amazon-bedrock".into(),
            base_url: "https://bedrock-runtime.us-east-1.amazonaws.com".into(), reasoning: true,
    ...
    fn user(content: Vec<ContentBlock>) -> Message {
        Message {
            role: Role::User, content, timestamp: 0,
    ...
    fn text(t: &str) -> ContentBlock { ContentBlock::Text { text: t.into(), text_signature: None } }
    ...
    fn skips_assistant_messages_with_no_renderable_content() {
        let assistant = Message {
            role: Role::Assistant, content: vec![text("")], timestamp: 0,
            api: Some("bedrock-converse-stream".into()), provider: Some("amazon-bedrock".into()),
    ...
    fn replaces_blank_tool_result_content_with_a_placeholder() {
        let tool_result = Message {
            role: Role::ToolResult, content: vec![text("")], timestamp: 0,
            api: None, provider: None, model: None, response_id: None, response_model: None,
            diagnostics: Vec::new(), usage: None, stop_reason: None, error_message: None,
            tool_call_id: Some("tool-1".into()), tool_name: Some("tool".into()), is_error: false, details: None,
        };
        let msgs = build(vec![tool_result]);
        assert_eq!(msgs.len(), 1);
        // The single tool-result block carries a "<empty>" text placeholder.
        let tr = msgs[0].content().iter().find_map(|b| match b {
            BedrockContent::ToolResult(tr) => Some(tr),
            _ => None,
        }).expect("a toolResult block");
        let placeholder = tr.content().iter().any(|c| matches!(c,
            aws_sdk_bedrockruntime::types::ToolResultContentBlock::Text(t) if t == "<empty>"));
        assert!(placeholder, "blank tool result must carry the <empty> placeholder");
    }
}
