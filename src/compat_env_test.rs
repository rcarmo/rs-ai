...
#[cfg(test)]
mod tests {
    use crate::registry::{register_api, stream, ApiProvider};
    ...
    impl ApiProvider for CaptureProvider {
        ...
        fn stream<'a>(
            ...
            *self.captured.lock().unwrap() = opts.api_key.clone();
            let msg = Message {
                role: Role::Assistant, content: vec![ContentBlock::Text { text: "ok".into(), text_signature: None }],
                timestamp: 0, api: Some(model.api.clone()), provider: Some(model.provider.clone()), model: Some(model.id.clone()),
            ...
            Box::pin(futures::stream::iter(vec![
                Event::Start { partial: msg.clone() },
                Event::Done { reason: StopReason::Stop, message: msg },
            ]))
        }
    }
    ...
    async fn dispatches_unknown_providers_through_the_api_registry_with_request_key() {
        ...
        register_api(Arc::new(CaptureProvider { captured: captured.clone() }));

        let model = Model {
            id: "test-model".into(), name: "Test Model".into(), api: CAPTURE_API.into(),
            provider: "custom-openai".into(), base_url: "https://example.test/v1".into(), reasoning: false,
            thinking_level_map: None, input: vec!["text".into()], cost: ModelCost::default(),
            context_window: 128000, max_tokens: 4096, headers: None, api_key: None, compat: Default::default(),
        };
        let ctx = Context {
        ...
        let opts = StreamOptions { api_key: Some("request-key".into()), ..Default::default() };
