...
mod tests {
    ...
    fn anthropic_normalizes_to_valid_64_char_id() {
        ...
        assert!(matches_id_charset(&id), "anthropic id has invalid chars: {id}");
    ...
    fn bedrock_normalizes_to_valid_64_char_id() {
        ...
        assert!(matches_id_charset(&id), "bedrock id has invalid chars: {id}");
    ...
    fn openai_normalizes_pipe_id_to_valid_40_char_call_id() {
        ...
        assert!(id.len() <= 40, "openai pipe call id must be <= 40: {}", id.len());
        // The call-id portion (before `|`) is preserved.
        assert!(id.starts_with("call_pAYbIr76hXIjncD9UE4eGfnS"), "openai keeps the call_id part: {id}");
    }
