...
mod tests {
    ...
    fn gemini_model(api: &str, provider: &str, id: &str) -> Model {
        Model {
            id: id.into(), name: "Gemini 3 Pro Preview".into(), api: api.into(), provider: provider.into(),
            base_url: "https://example.com".into(), reasoning: true, thinking_level_map: None,
            input: vec!["text".into()], cost: ModelCost::default(), context_window: 128000, max_tokens: 8192,
            headers: None, api_key: None, compat: Default::default(),
        }
    }
    ...
    fn tool_call(id: &str, command: &str, sig: Option<&str>) -> ContentBlock {
        ...
        ContentBlock::ToolCall { id: id.into(), name: "bash".into(), arguments: args, thought_signature: sig.map(|s| s.into()) }
    }
    ...
    fn ctx(model: &Model, assistant_id: &str, thought_signature: Option<&str>) -> Context {
        let assistant = Message {
            role: Role::Assistant,
            content: vec![tool_call("call_1", "echo hi", thought_signature), tool_call("call_2", "ls -la", None)],
            timestamp: 0,
            api: Some(model.api.clone()), provider: Some(model.provider.clone()), model: Some(assistant_id.into()),
        ...
        Context {
            ...
            messages: vec![
                Message { role: Role::User, content: vec![ContentBlock::Text { text: "Hi".into(), text_signature: None }],
    ...
    fn model_turn(payload: &Value) -> Value {
        payload["contents"].as_array().unwrap().iter()
            .find(|c| c.get("role").and_then(|r| r.as_str()) == Some("model")).cloned()
            .expect("a model turn")
    }

    fn function_call_sigs(turn: &Value) -> Vec<Option<String>> {
        turn["parts"].as_array().unwrap().iter()
            .filter(|p| p.get("functionCall").is_some())
            .map(|p| p.get("thoughtSignature").and_then(|v| v.as_str()).map(|s| s.to_string()))
            .collect()
    }
    ...
    fn no_skip_validator_for_unsigned_google_gen_ai_tool_calls() {
        ...
        let p = build_google_payload_public(&model, &ctx(&model, "other-model", None), &StreamOptions::default());
        ...
        assert!(!serde_json::to_string(&turn).unwrap().contains("Historical context"));
    }
    ...
    fn preserves_valid_thought_signature_for_same_provider_and_model() {
        ...
        let p = build_google_payload_public(&model, &ctx(&model, "gemini-3-pro-preview", Some(valid)), &StreamOptions::default());
        let turn = model_turn(&p);
        assert_eq!(function_call_sigs(&turn), vec![Some(valid.to_string()), None]);
    }
