...
#[cfg(test)]
mod tests {
    use crate::provider::codex::{is_ws_connection_limit_error, stream_codex, clear_ws_fallback, WS_CONNECTION_LIMIT_CODE};
    use crate::types::{Context, ContentBlock, Message, Model, ModelCost, Role, StreamOptions, Transport};
    ...
    fn detects_connection_limit_marker() {
        assert!(is_ws_connection_limit_error(&format!("{WS_CONNECTION_LIMIT_CODE}: too many")));
        assert!(!is_ws_connection_limit_error("WebSocket stream closed before response.completed"));
    ...
    async fn retries_websocket_once_on_connection_limit_then_succeeds() {
        ...
        tokio::time::timeout(Duration::from_secs(5), run).await.expect("codex stream timed out");
        let _ = server.await;

        assert!(!saw_error, "connection-limit retry must not surface an error");
        assert_eq!(text, "hello", "the retried WebSocket stream must produce the text");
    }
}
