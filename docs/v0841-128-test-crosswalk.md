...
Disposition split: **10 credential-gated live-matrix additions** are not executed in Rust because they require live Qwen Token Plan credentials; their generic stream/tool/token/abort/handoff/unicode behavior remains covered by existing deterministic provider suites, and the new Individual provider request/catalog contract is verified separately. **2 deterministic provider/request rows** are adapted directly (`qwen-token-plan-models`, `openai-completions-tool-choice`). **2 generator-policy rows** are adapted through the release-shard validator/extractor (`model-data-validation`, new `generate-models-strict`).
...
| `qwen-token-plan-models.test.ts` | ADAPTED | `src/tests/release/v0841_release_test.rs::{qwen_token_plan_individual_catalog_env_and_endpoint_match_v0841,qwen_token_plan_individual_reasoning_payloads_match_v0841}` | Individual provider registered; shares `QWEN_TOKEN_PLAN_API_KEY`; exact 7-model catalog; retired `qwen3.8-max-preview` omitted; `thinkingFormat=qwen` emits `enable_thinking` and supported `reasoning_effort`. |
| `model-data-validation.test.ts` | ADAPTED | `scripts/validate_release_model_data.py`, `scripts/extract_release_model_shards.py`, `src/tests/catalogs/model_data_validation_test.rs` | Existing strict shard validation remains; extractor now preserves official release-pinned `:batch` aliases only when exactly allowlisted. |
| `openai-completions-tool-choice.test.ts` | ADAPTED | `src/tests/release/v0841_release_test.rs::qwen_token_plan_individual_reasoning_payloads_match_v0841`, existing `src/tests/providers/openai/openai_completions_tool_choice_test.rs` | Deterministic provider/request row: Individual models use the same OpenAI-compatible request path; Qwen `enable_thinking` plus supported `reasoning_effort` is asserted without live credentials. |
| 10 credential-gated live-matrix additions: `abort`, `context-overflow`, `cross-provider-handoff`, `empty`, `image-tool-result`, `stream`, `tokens`, `tool-call-without-result`, `total-tokens`, `unicode-surrogate` | LIVE UNEXECUTED / GENERICALLY COVERED | Existing deterministic generic provider suites plus `src/tests/release/v0841_release_test.rs` provider/catalog contract | These upstream additions add `qwen-token-plan-individual` to live credential matrices. Rust does not execute live Qwen calls; shared OpenAI-compatible behaviors remain covered by existing deterministic stream/tool/token/abort/handoff/unicode tests, while Individual endpoint/env/catalog/request metadata is asserted separately. |
