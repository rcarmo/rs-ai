...
mod tests {
    ...
    fn openrouter_auto(base_url: &str) -> Model {
        Model {
            id: "openrouter/auto".into(), name: "OpenRouter Auto".into(), api: "openai-completions".into(),
            provider: "openrouter".into(), base_url: base_url.into(), reasoning: false,
            thinking_level_map: None, input: vec!["text".into()], cost: ModelCost::default(),
            context_window: 200000, max_tokens: 8192, headers: None, api_key: Some("test".into()),
    ...
    async fn surfaces_routed_chunk_model_on_response_model_without_changing_model() {
        ...
        assert_eq!(m.response_model.as_deref(), Some("anthropic/claude-opus-4.8"));
