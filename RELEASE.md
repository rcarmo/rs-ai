...
- Scope status: **ADAPTED / frame-wire corrective runtime candidate hosted CI passed; auditor acceptance pending**.
...
Frame-wire corrective local results: `cargo fmt --all -- --check` passed; `cargo build` passed; focused assistant-message-frame wire tests passed (`19 passed`); focused Responses terminal/serialized-error tests passed (`8 passed`); full `cargo test --all-targets --all-features` passed three consecutive times (`974 passed`, `0 failed`, `0 ignored` each); strict `cargo clippy --all-targets --all-features -- -D warnings` passed; `cargo check --no-default-features` and `cargo check --no-default-features --features bedrock` passed; `cargo test --no-default-features` passed (`845 passed`, doctest `1 passed`); `cargo test --no-default-features --features bedrock` passed (`974 passed`, doctest `1 passed`); metadata verifier passed (`text=1336 providers=39 apis=9 batchAliases=66 image=50`); metadata fault gates failed closed for `text-name` and `image-name`; provider/id comparator passed with text `upstream=1336 local=1336 missing=0 extra=0` and image `upstream=50 local=50 missing=0 extra=0`; manifest validator passed with `changedSha256=db461a56838926cf60d4ae0196ed98fcc215616dacff013ad8c235bb8ad9b83f` and `testCorpusSha256=56f8742065a4ad01d73e5aee53035324f2e7333a735222ab15db870819e29065`; manifest fault gates failed closed for both inventories; baseline delta verifier passed (`text=+72/-26/79 changed image=+0/-0/0 changed`) and `--fault baseline-record` failed closed; `make check` passed, generating/validating local SBOM `artifacts/sbom.cdx.json` with **278** dependency components and local checksum `eb07c61ca5e0dbc14403978a5e2d4bb6a7c71ceac0d8369c650f3362d4193b3a`; license self-tests/review and RustSec wrapper/cargo-audit passed; `git diff --check` passed. Frame-wire corrective runtime SHA `1be9dbbbd837067ee424bd942ab9efcef5a1a799` passed hosted GitHub Actions run `33894216176`, job `build-test-lint` (`101092907130`); artifact `rs-ai-sbom-1be9dbbbd837067ee424bd942ab9efcef5a1a799` (`9945092030`) has GitHub archive digest `sha256:033f92211435d4a018f512a93a1c248fde0452be9f204942da7d54c068fa4c2d`, downloaded archive SHA-256 `033f92211435d4a018f512a93a1c248fde0452be9f204942da7d54c068fa4c2d`, inner CI `sbom.cdx.json` SHA-256 `ebaba8ae05ace684f86e18bf8d6069415d26b2e147b51a7e16a336d0f73b7e8f`, checksum-file SHA-256 `bd3cb8b0a8a4482ac2663bb0b890ff3c31bf55c21efc51b3474fd71ec51d158f`, embedded revision `1be9dbbbd837067ee424bd942ab9efcef5a1a799`, **278** components, and expiry `2026-10-04T16:17:41Z`. This evidence is recorded in follow-up docs-only SHA; superseded corrective candidate `fccd2202deafbaebc091388149cc8d81b2f0801d` / CI `33892584822` and older rejected candidate `ae87c229cd0a76a3b1611279ebcb3ef4ddd4d0fa` / CI `33888972744` are retained only as superseded evidence; local pre-push SBOM digests are not CI SBOM digests.
