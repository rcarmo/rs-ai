...
mod tests {
    ...
    fn model() -> Model {
        Model {
            id: "gpt-4o-mini".into(), name: "GPT-4o Mini".into(), api: "openai-responses".into(),
            provider: "openai".into(), base_url: "https://api.openai.com/v1".into(), reasoning: false,
            thinking_level_map: None, input: vec!["text".into()], cost: ModelCost::default(),
            context_window: 128000, max_tokens: 16000, headers: None, api_key: None, compat: Default::default(),
        }
    }
    ...
    fn uses_no_tool_output_placeholder_for_empty_tool_results_without_images() {
        let assistant = Message {
            ...
            content: vec![ContentBlock::ToolCall {
                id: "tool-1".into(), name: "bash".into(), arguments: HashMap::new(), thought_signature: None,
            }],
            timestamp: 0,
            api: Some("openai-responses".into()), provider: Some("openai".into()), model: Some("gpt-4o-mini".into()),
        ...
        let tool_result = Message {
            ...
            tool_call_id: Some("tool-1".into()), tool_name: Some("bash".into()),
        ...
        let ctx = Context {
            ...
            messages: vec![
                Message {
                    role: Role::User, content: vec![ContentBlock::Text { text: "Run the command".into(), text_signature: None }],
                    timestamp: 0, api: None, provider: None, model: None, response_id: None,
                    response_model: None, diagnostics: Vec::new(), usage: None,
                    stop_reason: None, error_message: None,
                    tool_call_id: None, tool_name: None, is_error: false, details: None,
        ...
        let fco = input.iter().find(|i| i.get("type").and_then(|t| t.as_str()) == Some("function_call_output"))
