...
mod tests {
    ...
    use crate::types::{CacheRetention, Context, ContentBlock, Message, Model, ModelCompat, Role, StreamOptions};
    ...
    fn openai_model() -> Model { get_model("openai", "gpt-4o-mini").unwrap() }
    ...
    fn sets_prompt_cache_retention_24h_when_long() {
        let p = payload(&openai_model(), &session("session-456", Some(CacheRetention::Long)));
    ...
    fn omits_prompt_cache_fields_when_none() {
        let p = payload(&openai_model(), &session("session-789", Some(CacheRetention::None)));
    ...
    fn omits_prompt_cache_fields_for_non_openai_without_long_retention() {
        let model = proxy_model(ModelCompat { supports_long_cache_retention: Some(false), ..Default::default() });
        let p = payload(&model, &session("session-proxy", Some(CacheRetention::Long)));
        assert!(p.get("prompt_cache_key").is_none());
        assert!(p.get("prompt_cache_retention").is_none());
    }
    ...
    fn uses_pi_cache_retention_env_for_direct_openai() {
        ...
        unsafe { std::env::set_var("PI_CACHE_RETENTION", "long"); }
    ...
    async fn capture_headers(model: Model, opts: StreamOptions) -> HashMap<String, String> {
        ...
        reqs.last().unwrap().headers.iter()
            .map(|(k, v)| (k.as_str().to_string(), v.to_str().unwrap_or("").to_string())).collect()
    }

    fn affinity_model() -> Model {
        proxy_model(ModelCompat { send_session_affinity_headers: Some(true), ..Default::default() })
    }
    ...
    async fn sends_session_affinity_headers_when_enabled() {
        ...
        assert_eq!(h.get("session_id").map(String::as_str), Some("session-affinity"));
        assert_eq!(h.get("x-client-request-id").map(String::as_str), Some("session-affinity"));
        assert_eq!(h.get("x-session-affinity").map(String::as_str), Some("session-affinity"));
    }
    ...
    async fn explicit_headers_override_generated_session_affinity_headers() {
        let opts = StreamOptions {
            ...
            headers: Some(HashMap::from([
                ...
                ("x-session-affinity".to_string(), "override-affinity".to_string()),
            ])),
            ..Default::default()
        };
        let h = capture_headers(affinity_model(), opts).await;
        assert_eq!(h.get("session_id").map(String::as_str), Some("override-session"));
        assert_eq!(h.get("x-client-request-id").map(String::as_str), Some("override-request"));
        assert_eq!(h.get("x-session-affinity").map(String::as_str), Some("override-affinity"));
    }
}
