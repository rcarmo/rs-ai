...
mod tests {
    use crate::auth::{
        resolve_provider_auth, Credential, EnvAuthContext, InMemoryCredentialStore, OAuthAuth,
    ...
    fn model(provider: &str) -> Model {
        Model {
            id: "m".into(), name: "M".into(), api: "anthropic-messages".into(),
    ...
    fn cred(access: &str, expires: i64) -> OAuthCredential {
        OAuthCredential { access: access.into(), refresh: Some("r".into()), expires, account_id: None }
    }

    #[tokio::test]
    async fn anthropic_to_auth_derives_the_api_key_from_the_access_token() {
        let auth = AnthropicOAuth::new().to_auth(&cred("token", 0)).await.unwrap();
    ...
    async fn anthropic_refresh_exchanges_the_refresh_token_and_returns_a_typed_credential() {
        ...
        Mock::given(method("POST"))
            ...
            .mount(&server)
            .await;
        let oauth = AnthropicOAuth { token_url: Some(format!("{}/oauth/token", server.uri())) };
        let refreshed = oauth.refresh(&OAuthCredential {
            access: "old".into(), refresh: Some("old-r".into()), expires: 0, account_id: None,
        }).await.unwrap();
    ...
    async fn resolves_stored_anthropic_oauth_credentials_via_the_resolver() {
        ...
        store.modify::<_, _, std::convert::Infallible>("anthropic", |_| async {
            Ok(Some(Credential::OAuth(OAuthCredential {
                access: "oauth-access-token".into(), refresh: Some("r".into()),
                expires: now_millis() + 60_000, account_id: None,
            })))
        }).await.unwrap();
        let provider = ProviderAuth { api_key: None, oauth: Some(Box::new(AnthropicOAuth::new())) };
        let ctx = EnvAuthContext::new();
        let result = resolve_provider_auth("anthropic", &provider, &model("anthropic"), &store, &ctx, None)
            .await.unwrap().unwrap();
