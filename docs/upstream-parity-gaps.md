...
| API/provider runtime source: `src/api/{anthropic-messages,azure-openai-responses,bedrock-converse-stream,constrained-sampling,google-generative-ai,google-shared,google-vertex,mistral-conversations,openai-codex-responses,openai-completions,openai-responses-shared,openai-responses,openrouter-images}.ts` | 13 | PORTED / N/A where already generic | constrained-sampling and provider request-shape deltas covered by existing generic schema/payload paths plus v0.82.0 tests; OpenAI-completions tool-call ID uniqueness and OpenCode Go Responses are explicitly tested. |
| OAuth/auth runtime: `src/auth/oauth/kimi-coding.ts`, `src/auth/oauth/load.ts`, `src/auth/oauth/openrouter.ts`, `src/bun-oauth.ts` | 4 | PORTED / N/A | Kimi/OpenRouter OAuth are credential/browser helper surfaces; deterministic auth semantics remain covered where Rust exposes provider auth. Bun CLI glue is N/A. |
...
| Shared retry utilities: `src/utils/provider-retry.ts`, `src/utils/retry.ts` | 2 | PORTED | Rust retry classifier plus production `retry_assistant_call` includes abortable provider retry/backoff semantics; abort-after-scheduled-retry reports unsuccessful. |
