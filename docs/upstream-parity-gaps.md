...
Status: **ADAPTED / frame-wire corrective runtime candidate locally green; replacement hosted CI pending**. The range changes **51** `packages/ai` paths: 19 source/scripts paths, 29 tests, and 3 package/docs paths. Whole-corpus filename evidence is in `docs/v0850-142-test-crosswalk.md`: **142/142** upstream `packages/ai/test/*.test.ts` files accounted, with executable validator `scripts/validate_v0850_manifests.py`.
