...
mod tests {
    ...
    fn sends_default_max_tokens_as_model_max_tokens() {
        ...
        assert_eq!(
            ...
            serde_json::json!(gpt_4o_mini().max_tokens)
        );
    }
    ...
    fn clamps_default_max_tokens_to_remaining_context() {
        ...
        let ctx = Context {
            system_prompt: None,
            tools: Vec::new(),
            messages: vec![user(&"x".repeat(8000))],
        };
    ...
    fn still_emits_tools_empty_array_when_conversation_has_tool_history() {
        ...
        assert_eq!(
            p["tools"],
            serde_json::json!([]),
            "tool history must keep an empty tools array"
        );
    }
    ...
    fn cloudflare_gateway_resolves_compat_base_url_and_cf_aig_auth() {
        ...
        assert_eq!(
            headers.get("cf-aig-authorization").unwrap(),
            "Bearer cf-token"
        );
        assert!(
            headers.get("authorization").is_none(),
            "primary Authorization must not be set for the gateway"
        );
    }
