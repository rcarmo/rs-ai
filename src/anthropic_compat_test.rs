...
mod tests {
    ...
    use crate::types::{Context, ContentBlock, Message, Model, ModelCost, ModelCompat, Role, StopReason, StreamOptions, Tool};
    ...
    fn eager_model(base_url: &str, compat: ModelCompat) -> Model {
        Model {
            id: "claude-opus-4-8".into(), name: "Claude Opus 4.8".into(), api: "anthropic-messages".into(),
            provider: "test-anthropic".into(), base_url: base_url.into(), reasoning: true, thinking_level_map: None,
            input: vec!["text".into()], cost: ModelCost::default(), context_window: 200000, max_tokens: 32000,
            headers: None, api_key: Some("test-key".into()), compat,
        }
    }
    ...
    async fn capture(compat: ModelCompat, ctx: Context) -> (Value, Option<String>) {
        ...
        let opts = StreamOptions { cache_retention: Some(crate::types::CacheRetention::None), ..Default::default() };
        ...
        let beta = req.headers.get("anthropic-beta").map(|v| v.to_str().unwrap().to_string());
        (body, beta)
    }

    fn force_adaptive() -> ModelCompat {
        ModelCompat { force_adaptive_thinking: Some(true), ..Default::default() }
    }
    ...
    async fn sends_per_tool_eager_input_streaming_by_default() {
        ...
        assert!(beta.is_none(), "no anthropic-beta header by default, got {beta:?}");
    }

    #[tokio::test]
    async fn uses_legacy_fine_grained_beta_when_eager_disabled() {
        let compat = ModelCompat { force_adaptive_thinking: Some(true), supports_eager_tool_input_streaming: Some(false), ..Default::default() };
        let (body, beta) = capture(compat, user_ctx(vec![lookup_tool()])).await;
        assert!(body["tools"][0].get("eager_input_streaming").is_none());
        assert_eq!(beta.as_deref(), Some("fine-grained-tool-streaming-2025-05-14"));
    }
    ...
    fn marks_builtin_anthropic_models_that_use_adaptive_thinking() {
        let mut flagged: Vec<String> = list_models(None).into_iter()
            .filter(|m| m.api == "anthropic-messages" && m.compat.force_adaptive_thinking == Some(true))
        ...
        for expected in [
            ...
            assert!(flagged.iter().any(|f| f == expected), "missing adaptive-thinking model {expected}; got {flagged:?}");
        }
        let re = regex_lite_matches;
        for id in &flagged {
            assert!(re(id), "flagged model {id} does not match the expected opus/sonnet/fable pattern");
        }
    }
    ...
    fn mimo_model(allow_empty_signature: Option<bool>) -> Model {
        Model {
            id: "mimo-v2.5-pro".into(), name: "MiMo-V2.5-Pro".into(), api: "anthropic-messages".into(),
            provider: "xiaomi-token-plan-ams".into(), base_url: "http://127.0.0.1:9/anthropic".into(),
            reasoning: true, thinking_level_map: None, input: vec!["text".into()], cost: ModelCost::default(),
            context_window: 1048576, max_tokens: 1024, headers: None, api_key: None,
            compat: ModelCompat { allow_empty_signature, ..Default::default() },
        }
    }
    ...
    fn thinking_ctx(signature: &str) -> Context {
        let assistant = Message {
            role: Role::Assistant,
            content: vec![ContentBlock::Thinking { thinking: "internal reasoning".into(), thinking_signature: Some(signature.into()), redacted: false }],
            timestamp: 0,
            api: Some("anthropic-messages".into()), provider: Some("xiaomi-token-plan-ams".into()), model: Some("mimo-v2.5-pro".into()),
        ...
        Context {
            ...
            messages: vec![
                Message { role: Role::User, content: vec![ContentBlock::Text { text: "first".into(), text_signature: None }],
                    ...
                    tool_call_id: None, tool_name: None, is_error: false, details: None },
                assistant,
                Message { role: Role::User, content: vec![ContentBlock::Text { text: "second".into(), text_signature: None }],
                    timestamp: 0, api: None, provider: None, model: None, response_id: None, response_model: None,
                    diagnostics: Vec::new(), usage: None, stop_reason: None, error_message: None,
                    tool_call_id: None, tool_name: None, is_error: false, details: None },
            ],
    ...
    fn assistant_content(p: &Value) -> &Value {
        p["messages"].as_array().unwrap().iter()
            .find(|m| m.get("role").and_then(|r| r.as_str()) == Some("assistant")).map(|m| &m["content"]).unwrap()
    }

    #[test]
    fn converts_empty_signature_thinking_to_text_by_default() {
        let p = build_anthropic_payload(&mimo_model(None), &thinking_ctx(""), &StreamOptions::default());
        assert_eq!(*assistant_content(&p), json!([{"type": "text", "text": "internal reasoning"}]));
    }

    #[test]
    fn preserves_empty_signature_thinking_when_allow_empty_signature_enabled() {
        let p = build_anthropic_payload(&mimo_model(Some(true)), &thinking_ctx(" "), &StreamOptions::default());
        assert_eq!(*assistant_content(&p), json!([{"type": "thinking", "thinking": "internal reasoning", "signature": ""}]));
    }
}
